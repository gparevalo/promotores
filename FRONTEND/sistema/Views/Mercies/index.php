...
<body>
    ...
    <section class="wrap-general">
        ...
        <section class="resultados">
            <p class="title-category"><span class="icon-briefcase ico-d399f88"></span>Comercios <!-- <span class="nros"> | 3 898 registros</span> --></p>
            ...
            <div id="items-container">
                <?php foreach ($data->mercies as $row) { ?>
                    <section class="item-contact">
                        ...
                        <section class="itemcontact-right">
                            ...
                            <a href="<?= base_url() ?>/Comercios/detalle/<?= $titulo ?>-<?= $row->idProspect ?>" class="icon-info ico-dj7f8s7s"></a>
            ...
            <section class="wrap-viewpl <?= count($data->mercies) > 20 ? '' : 'ocult'; ?>">
                <button id="show-more-btn" class="bt-next1">Ver 20 más<span class="icon-eye ico-djde837"></span></button>
                <span class="restante"><?= count($data->mercies); ?> por ver</span>
            </section>
        </section>
    ...
    <?php foreach ($data->mercies as $row) { ?>
        <div class="awesome-nav" id="view-info-<?= $row->idProspect ?>">
            ...
            <div class="wrap-nav-int">
                ...
                <?php $titulo = create_slug($row->category . " " . $row->nameCProspect) ?>
                <a href="<?= base_url() ?>/Comercios/detalle/<?= $titulo ?>-<?= $row->idProspect ?>" class="bt-next1">Ver más información <span class="icon-eye ico-djde837"></span></a>
            </div>
        </div>
    <?php } ?>
    ...
    <div class="padd-footer">
        ...
        <script>
            document.getElementById('exportarExcel').addEventListener('click', function() {
                ...
                XLSX.utils.book_append_sheet(libroDeExcel, hojaDeExcelConEnlaces, "Comercios");

                var fecha = new Date();
                var fechaFormateada = fecha.getFullYear() + '-' + (fecha.getMonth() + 1).toString().padStart(2, '0') + '-' + fecha.getDate().toString().padStart(2, '0');

                var nombreArchivo = "Comercios_" + fechaFormateada + ".xlsx";
