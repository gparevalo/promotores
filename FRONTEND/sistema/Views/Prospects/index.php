    ...
    <section class="wrap-general">
        ...
        <section class="resultados">
            ...
            <section class="wrap-viewpl <?= count($data->prospects) > 20 ? '' : 'ocult'; ?>">
                ...
                <span class="restante"><?= count($data->prospects); ?> En total</span>
            </section>
        </section>
    ...
    <?php foreach ($data->prospects as $row) { ?>
        <div class="awesome-nav" id="view-info-<?= $row->idProspect ?>">
            ...
            <div class="wrap-nav-int">
                ...
                <?php $titulo = create_slug($row->category . " " . $row->nameCProspect) ?>
                <a href="<?= base_url() ?>/Prospectos/detalle/<?= $titulo ?>-<?= $row->idProspect ?>" class="bt-next1">Editar <span class="icon-edit-3 ico-djde837"></span></a>
            </div>
        </div>
    <?php } ?>
        ...
        <script>
            ...
            document.getElementById('exportarExcel').addEventListener('click', function() {
                ...
                var hojaDeExcelConEnlaces = XLSX.utils.aoa_to_sheet([
                    ["Fecha", "Categoría", "Nombre", "Promotor", "Aprobador", "Instalador", "Nombre completo", "Teléfono", "Dirección", "¿Desea el servicio?", "Es socio", "Me enteré por:", "RUC", "Email", "URL DE IMAGEN", "Estado"]
                ].concat(datosFiltradosConEnlaces));

                XLSX.utils.book_append_sheet(libroDeExcel, hojaDeExcelConEnlaces, "Prospects");

                var fecha = new Date();
                var fechaFormateada = fecha.getFullYear() + '-' + (fecha.getMonth() + 1).toString().padStart(2, '0') + '-' + fecha.getDate().toString().padStart(2, '0');

                var nombreArchivo = "Prospectos_" + fechaFormateada + ".xlsx";
